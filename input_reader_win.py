...
def sample_input_reader(mario_inputs):
    ...
    if config['keyboard_control']:
        mario_inputs.stickX = input_value['LEFT']*1 - input_value['RIGHT']*1
        mario_inputs.stickY = input_value['UP']*1 - input_value['DOWN']*1
        ...
        except (OSError, BrokenPipeError, ValueError) as pipe_error:
            print(f"[LibSM64 Addon] Input reader stream disconnected ({pipe_error}). Dropping frame tick.")
